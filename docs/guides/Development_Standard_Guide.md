...
## 5. NLP 및 데이터 전처리 표준 (NLP & Data Standards)

프로젝트의 데이터 품질 유지를 위해 아래 텍스트 처리 규칙을 준수합니다.

### 5.1. 키워드 추출 규칙
- **영문 키워드:** 최소 3자 이상인 단어만 추출하여 형태소 파편(di, se 등)을 방지합니다.
- **불용어 처리:** `ENGLISH_STOPWORDS`, `KOREAN_STOPWORDS` 리스트에 정의된 기능어(조사, 전치사 등)는 키워드 집계에서 제외합니다.
- **정규화:** 모든 영문 키워드는 소문자로 변환하여 동일 단어로 집계합니다.

### 5.2. 텍스트 정제 (Cleaning)
- 데이터 수집 시 HTML 태그, URL, 불필요한 특수문자는 반드시 제거하여 AI 분석 효율을 높입니다.
- 연속된 공백 및 줄바꿈은 단일 공백으로 치환합니다.
